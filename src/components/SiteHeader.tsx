          ...
          <div className="hidden items-center gap-6 sm:flex">
            {navLinks.slice(0, 4).map((link) => (
              <Link
                ...
                activeProps={{
                  className: "text-navy font-semibold border-b-2 border-navy",
                }}
                inactiveProps={{ className: "text-navy-deep/70 border-b-2 border-transparent" }}
                className="pb-0.5 text-sm transition-colors hover:text-navy"
              >
                {link.label}
              </Link>
            ))}
